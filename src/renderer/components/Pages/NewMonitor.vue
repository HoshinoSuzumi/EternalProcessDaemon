<template>
  <div>
    <ContentBlock title="新建守护项目" blank>
      ...
      <SettingItem title="最短运行时间(ms)" detail="如果目标进程启动后在这个时间之前退出，那么守护进程也将退出" tag="monitorExecPath" type="input"/>
      <SettingItem title="异常重启间隔(ms)" detail="如果进程存活时间小于最短运行时间，重新拉起进程的时间间隔" tag="monitorExecPath" type="input"/>
